...
(chat, code completion, and Copilot Coding Agent). They encode the hard
contracts the `dynatrace_aws_monitoring_configuration` resource must keep.

When the Coding Agent is invoked from an issue created by
`.github/workflows/da-aws-schema-watch.yml`, also load and obey
[`/.github/agents/da-aws-bump/AGENT_INSTRUCTIONS.md`](./agents/da-aws-bump/AGENT_INSTRUCTIONS.md).
...
These are non-negotiable. A PR that violates any of them is rejected by
`verify.sh` and must be revised.
